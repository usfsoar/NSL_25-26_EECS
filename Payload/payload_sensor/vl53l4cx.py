...
* Author(s): Carter Nelson (author of VL53L1X and VL53L4CD libraries)
             Pat Daderko (modified VL53L1X and VL53L4CD libraries to work with VL53L4CX)
...
* Adafruit `VL53L4CX Time of Flight Distance Sensor - ~1 to 6000mm
  <https://www.adafruit.com/product/5425>`_
...
__repo__ = "https://github.com/pdaderko/CircuitPython_VL53L4CX.git"

_VL53L4CX_I2C_SLAVE_DEVICE_ADDRESS = const(0x0001)
_VL53L4CX_VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND = const(0x0008)
...
_VL53L4CX_RESULT__RANGE_STATUS = const(0x0089)
_VL53L4CX_RESULT_SIGMA = const(0x0092)
_VL53L4CX_RESULT__FINAL_CROSSTALK_CORRECTED_RANGE_MM_SD0 = const(0x0096)
_VL53L4CX_IDENTIFICATION__MODEL_ID = const(0x010F)
...
class VL53L4CX:
    ...
    def _sensor_init(self):
        ...
        self.stop_ranging()
        self._write_register(_VL53L4CX_VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND, b"\x09")
        self._write_register(0x0B, b"\x00")

    @property
    def model_info(self):
        """A 3 tuple of Model ID, Module Type, and Mask Revision."""
        info = self._read_register(_VL53L4CX_IDENTIFICATION__MODEL_ID, 3)
        return (info[0], info[1], info[2])  # Model ID, Module Type, Mask Rev

    @property
    def distance(self):
        """The distance in units of centimeters."""
        dist = self._read_register(_VL53L4CX_RESULT__FINAL_CROSSTALK_CORRECTED_RANGE_MM_SD0, 2)
        dist = struct.unpack(">H", dist)[0]
        return dist / 10

    @property
    def range_status(self):
        status = self._read_register(_VL53L4CX_RESULT__RANGE_STATUS, 1)
    ...
    def set_address(self, new_address):
        ...
        multiple VL53L sensors on the same I2C bus (SDA & SCL pins). See also the
