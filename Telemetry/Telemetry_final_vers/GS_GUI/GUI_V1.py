...
def parse_line(line: str):
    """
    Returns (kind, payload_dict) or None.
    Handles both GS-wrapped lines and raw sensor strings.
    """
    line = line.strip()
    if not line:
        return None
    ...
    # Extract raw sensor string
    raw = None
    m = _TLM_RE.match(line)
    if m:
        raw = m.group(1).strip()
        m2 = _TLM_HDR_RE.match(raw)
        if m2:
            raw = m2.group(1).strip()
    elif line and line[0] in '0123':
        raw = line
    ...
    try:
        ...
        # IMU  (type 0) : ts,ax,ay,az, lx,ly,lz, gx,gy,gz, qw,qx,qy,qz, wx,wy,wz
        if ptype == 0 and len(fields) >= 17:
            return ('IMU', {
                't': t, 'ts': ts,
                'ax': float(fields[1]),  'ay': float(fields[2]),  'az': float(fields[3]),
                'lx': float(fields[4]),  'ly': float(fields[5]),  'lz': float(fields[6]),
                'gx': float(fields[7]),  'gy': float(fields[8]),  'gz': float(fields[9]),
                'qw': float(fields[10]), 'qx': float(fields[11]), 'qy': float(fields[12]), 'qz': float(fields[13]),
                'wx': float(fields[14]), 'wy': float(fields[15]), 'wz': float(fields[16]),
            })

        # ALT  (type 1) : ts,altitude,temp,pressure
        if ptype == 1 and len(fields) >= 4:
            return ('ALT', {
                't': t, 'ts': ts,
        ...
        # GPS  (type 2) : ts,nmea...
        if ptype == 2:
            nmea = ','.join(fields[1:]) if len(fields) > 1 else ''
            return ('GPS', {'ts': ts, 'nmea': nmea.strip()})

        # KALMAN (type 3) : ts,alt,vel,acc
