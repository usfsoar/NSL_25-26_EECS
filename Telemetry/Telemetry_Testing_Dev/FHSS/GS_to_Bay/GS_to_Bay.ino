...
static void enqueueRadioCommand(String line) {
  ...
  if (line.startsWith("freq ")) {
    ...
    char msg[64];
    snprintf(msg, sizeof(msg), "%lu,FREQ,%.3f", (unsigned long)freqTxn.seq, freqTxn.f_new);
    radioSendAscii(msg);
  ...
  if (line == "ping") {
    uint32_t seq = g_seq++;
    char msg[32];
    snprintf(msg, sizeof(msg), "%lu,PING", (unsigned long)seq);
    radioSendAscii(msg);
  ...
  if (line == "reboot") {
    uint32_t seq = g_seq++;
    char msg[32];
    snprintf(msg, sizeof(msg), "%lu,REBOOT", (unsigned long)seq);
    radioSendAscii(msg);
    Serial.printf("[GS] Sent %s\n", msg);
    return;
  }
