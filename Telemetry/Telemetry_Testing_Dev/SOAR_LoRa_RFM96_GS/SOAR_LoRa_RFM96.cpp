  ...
  while (true) {
    if (rf95.available()) {
      if (rf95.recv(buf, &len)) {
        unpackData(buf, len, outType, outData, outCap);
        return true;
      } else return false;
    }
...
bool SOAR_LoRa_RFM96::sendTypedString(DataType type, const char* data) {
    uint8_t txbuf[RH_RF95_MAX_MESSAGE_LEN];
    size_t txlen = packData(txbuf, sizeof(txbuf), type, data);
    if (txlen == 0) return false; // Packing failed
    rf95.send(txbuf, (uint8_t)txlen);
    rf95.waitPacketSent();
    return true; // Successfully packed and sent
}
