...
/*
LORA NOTES
V1_2_SOAR_RTOS_LORA.h and V1_2_SOAR_RTOS_LORA.cpp are from LORA_Receive
V1_3_SOAR_RTOS_LORA.h and V1_3_SOAR_RTOS_LORA.cpp are from LORA_Transmit
V1_4_SOAR_RTOS_LORA.h and V1_4_SOAR_RTOS_LORA.cpp is V1_2 AND V1_3 COMBINED
V1_4_SOAR_RTOS_LORA.h and V1_4_SOAR_RTOS_LORA.cpp are IN THE FINAL F24_V1_AV_BAY FOLDER
*/
...
bool SOAR_Lora::read(int* address, int* length, byte** data, int* rssi, int* snr) {
    *length = 0; // Default length to 0
    ...
    // Check if there is data available on the LoRa serial
    if (!loraSerial->available()) {
        return false;
    }

    // Measure the time when reading is started, and set a timeout
    unsigned long startTime = xTaskGetTickCount();
    const unsigned long TIMEOUT = 5000; // Timeout in milliseconds

    // While the time passed is less than the timeout
    while ((xTaskGetTickCount() - startTime) < TIMEOUT) {
        // If the LoRa serial has data available
        if (loraSerial->available()) {
            ...
            if (loraSerial->peek() == '+') {
                ...
                // If the response starts with "+RCV="
                if (response.startsWith("+RCV=")) {
                    // Remove "+RCV=" and read the address
                    response.remove(0, 5);
                    *address = response.toInt();

                    // Read up to the next comma and extract the length of the message
                    String lengthStr = loraSerial->readStringUntil(',');
                    *length = lengthStr.toInt();
                    ...
                    if (*length > 0) {
                        ...
                        *data = new byte[*length];
                        ...
                        // Read the data
                        int bytesRead = 0;
                        unsigned long dataStartTime = xTaskGetTickCount();
                        const unsigned long DATA_TIMEOUT = 1000; // Timeout in milliseconds for reading data

                        while ((xTaskGetTickCount() - dataStartTime) < DATA_TIMEOUT && bytesRead < *length) {
                            if (loraSerial->available()) {
                        ...
                        if (bytesRead < *length) {
                            Serial.println("Not enough data available within timeout. Bytes read: " + String(bytesRead));
                            delete[] * data;
                            *data = nullptr;
                            *length = bytesRead;
                            return false;
                        ...
                        String rssiStr = loraSerial->readStringUntil(','); // Read up to the next comma
                        ...
                        *snr = snrStr.toInt(); // Read the SNR
                        ...
                        Serial.println("Invalid length received: " + String(*length));
                    }
                }
                else {
    ...
    Serial.println("Timeout reached or no valid '+RCV' message.");
...
void SOAR_Lora::endPacketWTime(int address) {
    this->sendLong((xTaskGetTickCount() / portTICK_PERIOD_MS));
...
void SOAR_Lora::stringPacketWTime(const char* str, int address) {
    ...
    beginPacket();
    sendChar(str);
    sendLong((xTaskGetTickCount() / portTICK_PERIOD_MS));
    endPacket(address);
}
...
void SOAR_Lora::handleQueue() {
    if (!messageQueue.empty() && ((xTaskGetTickCount() / portTICK_PERIOD_MS) - lastSentTime >= MIN_QUEUE_TIME)) {
        ...
        messageQueue.pop();
        lastSentTime = (xTaskGetTickCount() / portTICK_PERIOD_MS);
    }
}
...
String SOAR_Lora::sendATCommand(const byte* command, int length, unsigned long timeout) {
    ...
    while ((xTaskGetTickCount() / portTICK_PERIOD_MS) - startTime < timeout) { // Wait for a response
        if (loraSerial->available()) {
            char c = loraSerial->read();
            Serial.write(c);
            result += c;  // append to the result string
            last_read_time = (xTaskGetTickCount() / portTICK_PERIOD_MS);
        }
        if (last_read_time != 0 && (xTaskGetTickCount() / portTICK_PERIOD_MS) - last_read_time > 100) {
