...
#define FLIGHT_DATA_SIZE 20  // Number of float/ulong values per flight
...
#define OFFSET_STATE_MACHINE 15
...
void setup() {
  ...
  if (!EEPROM.begin(EEPROM_SIZE)) {
    ...
    //   Serial.println("Warning: Invalid state read from EEPROM, resetting to READY.");
    //   currentState = READY;
    // }
...
void loop() {
  ...
  if (currentState == LAUNCHED || currentState == DESCENDING) {
    ...
    if (now - EELastLogTime >= 2000) {  // delay log to approx every 100ms (adjust rate as needed)
      saveDataToEEPROM();
      EELastLogTime = now;
      Serial.println("save to eeprom, 2 sec");
    }
  ...
  switch (currentState) {
    case READY:
      ...
      //xbee
      // char message[100];
      // sprintf(message, "1, 4, GForce, %.2f, Altitude, %.2f, Velocity, %.2f", emaGForce, emaAltitude, emaVelocity);
      // xbee.send(message);
    ...
    case LAUNCHED:
      ...
      if (!descentStarted && apogee > 1 && emaAltitude < (apogee - DESCENT_ALTITUDE_THRESHOLD)) {
      ...
      if (stableReadingDescent >= STABLE_READINGS_FOR_DESCENT) {
        ...
        //xbee
        // char message[100];
        // sprintf(message, "1, 4, GForce, %.2f, Apogee, %.2f, Velocity, %.2f", emaGForce, apogee, emaVelocity);
        // xbee.send(message);
      }
    ...
    case DESCENDING:
      ...
      if (emaVelocity < LANDING_VEL_THRESHOLD && abs(emaGForce - 1.0) < LANDING_ACCEL_THRESHOLD && emaAltitude < -0.3) {
        stableReadingLand++;
      } else {
...
void readAllSensors(float& currentAltitude, float& currentTemp,
  ...
  if (!soar_barometer.isReady() || isnan(currentAltitude)) {
    currentAltitude = soar_bme.getAltitude();// - bmeGroundLevel;
  }
...
void handleLandingSequence(float finalVelocityReading, float finalGravityReading[3]) {
  ...
  // char* message[300];
  // sprintf(message, "1, 4, Summary, %s", summary.c_str());
  // xbee.send(message);
...
void saveDataToEEPROM() {
  ...
  EEPROM.put(baseAddress + OFFSET_STATE_MACHINE * sizeof(float), (int)currentState);
...
void displayAllFlightData() {
  ...
  for (int i = 0; i < MAX_FLIGHTS; i++) {
    ...
    EEPROM.get(baseAddress + OFFSET_STATE_MACHINE * sizeof(float), currentState);
...
void restoreLastFlightData() {
  ...
  if (storedFlightTime > 0) {
    ...
    if (currentState != READY && currentState != LAUNCHED && currentState != DESCENDING && currentState != LANDED) {
      Serial.println("Warning: Restored flight state invalid. Resetting to READY.");
      currentState = READY;
    }

    Serial.println("Restored Flight Progress");
