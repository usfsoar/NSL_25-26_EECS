...
void readAllSensors(float& currentAltitude, float& bmeAltitude, float& currentTempBmp, float& currentTempBme,
                    float& currentPressureBmp, float& currentPressureBme, float& currentDecibels,
...
void updateFlightExtremes(float currentGForce, float currentRpm, float currentTempBmp, float currentPressureBmp, float currentDecibels, float imuVelocityMag);
...
void logFlightDataToSD(unsigned long timestamp, float currentAltitude, float bmeAltitude, float currentTempBmp, float currentTempBme,
                       float currentRpm, float currentZAccel, float currentGForce, float currentVelocity, float currentDecibels,
                       float currentPressureBme, float currentPressureBmp);  // Modified log function
...
const float LANDING_ACCEL_THRESHOLD = 1.2;  // G force threshold difference from 1G to detect landing
...
void loop() {
  ...
  // --- Sensor Readings ---
  float currentAltitude, bmeAltitude, currentTempBmp, currentTempBme, currentPressureBmp, currentPressureBme, currentDecibels;
  float acceleration[3], linearAccel[3], gravity[3], velocity[1], rpmReading[1];
  float currentGForce, currentVelocityMag;

  readAllSensors(currentAltitude, bmeAltitude, currentTempBmp, currentTempBme,
                 currentPressureBmp, currentPressureBme, currentDecibels,
  ...
  if (currentState == LAUNCHED || currentState == DESCENDING) {
    ...
    updateFlightExtremes(currentGForce, rpmReading[0], currentTempBmp, currentPressureBmp, currentDecibels, currentVelocityMag);
    ...
    if (now - lastLogTime >= 100) {  // delay log to approx every 100ms (adjust rate as needed)
      logFlightDataToSD(now - flightStartTime, currentAltitude, bmeAltitude, currentTempBmp, currentTempBme,
                        rpmReading[0], linearAccel[2], currentGForce, velocity[0], currentDecibels,
                        currentPressureBme, currentPressureBmp);
      lastLogTime = now;
    }
  ...
  switch (currentState) {
    ...
    case DESCENDING:
      ...
      if (emaVelocity < LANDING_VEL_THRESHOLD && abs(emaGForce - 1.0) < LANDING_ACCEL_THRESHOLD && emaAltitude < 6.0) {
        stableReadingLand++;
      } else {
...
void readAllSensors(float& currentAltitude, float& bmeAltitude, float& currentTempBmp, float& currentTempBme,
  ...
  currentAltitude = soar_barometer.get_altitude() - groundLevel;  // Relative altitude
  bmeAltitude = soar_bme.getAltitude() - bmeGroundLevel;          // Relative altitude
...
void updateFlightExtremes(float currentGForce, float currentRpm, float currentTempBmp, float currentPressureBmp, float currentDecibels, float imuVelocityMag) {
  ...
  if (currentTempBmp < minTemperature) minTemperature = currentTempBmp;
  if (currentTempBmp > maxTemperature) maxTemperature = currentTempBmp;

  // Update Pressure Min/Max (using BMP pressure)
  if (currentPressureBmp < minPressure) minPressure = currentPressureBmp;
  if (currentPressureBmp > maxPressure) maxPressure = currentPressureBmp;
...
void logFlightDataToSD(unsigned long timestamp, float currentAltitude, float bmeAltitude, float currentTempBmp, float currentTempBme,
                       float currentRpm, float currentZAccel, float currentGForce, float currentVelocity, float currentDecibels,
                       float currentPressureBme, float currentPressureBmp) {
  ...
  int writtenChars = snprintf(dataBuffer, sizeof(dataBuffer),
                              "%lu,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.3f,%.3f,%.2f,%.2f,%.0f\n",
                              timestamp,           // Time(ms) since launch
                              currentAltitude,     // Alt_BMP(m)
                              bmeAltitude,         // Alt_BME(m)
                              currentTempBmp,      // Temp_BMP(C)
                              currentTempBme,      // Temp_BME(C)
                              currentRpm,          // RPM
                              currentZAccel,       // Accel Z (m/s^2)
                              currentGForce,       // G-Force
                              currentVelocity,     // Vel_IMU(m/s) - Use the component directly? Or magnitude? Check need.
                              currentDecibels,     // dB
                              currentPressureBme,  // Press_BME(Pa)
                              currentPressureBmp   // Press_BMP(Pa)
  );
