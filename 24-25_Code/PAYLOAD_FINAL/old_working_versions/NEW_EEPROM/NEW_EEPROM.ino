...
const int STABLE_READINGS_FOR_LANDING_VELOCITY = 3;
...
void setup() {
  ...
  if (flightData.currentState == 0.0) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      Serial.println(soar_barometer.get_altitude());
      Serial.print("BME Alt: ");
      Serial.println(soar_bme.getAltitude());
      delay(500);
    }
  ...
  if (flightData.currentState == 0.0) {
    ...
    if (lastFlightIndex < 0) lastFlightIndex = SLOT_COUNT - 1;
...
void loop() {
  ...
  switch ((int)flightData.currentState) {
    ...
    case 2:
      ...
      if (flightData.emaVelocity < LANDING_VEL_THRESHOLD && abs(flightData.emaGForce - 1.0) <   && flightData.emaAltitude < LANDING_ALTITUDE_THRESHOLD) {  // Adjusted altitude check slightly
        stableReadingLand++;
      } else {
        stableReadingLand = 0;  // Reset count if conditions aren't met
      }

      if (stableReadingLand >= STABLE_READINGS_FOR_LANDING_VELOCITY) {
...
void updateFlightExtremes(float currentGForce, float currentRpm, float currentAltitude, float currentTemp, float currentPressure, float currentDecibels, float imuVelocityMag) {
  // Update Max G-Force
  if (currentGForce > flightData.maxGForce) flightData.maxGForce = currentGForce;

  // Update Max Absolute RPM
  if (currentRpm > flightData.maxRPM) flightData.maxRPM = currentRpm;
  updateMaxSustainedRPM(currentRpm);  // Update max sustained RPM

  if (currentAltitude > flightData.apogee) flightData.apogee = currentAltitude;

  // Update Temp Min/Max (using BMP temp)
  if (currentTemp < flightData.minTemperature) flightData.minTemperature = currentTemp;
  if (currentTemp > flightData.maxTemperature) flightData.maxTemperature = currentTemp;

  // Update Pressure Min/Max (using BMP pressure)
  if (currentPressure < flightData.minPressure) flightData.minPressure = currentPressure;
  if (currentPressure > flightData.maxPressure) flightData.maxPressure = currentPressure;

  // Update Max Decibels
  if (currentDecibels > flightData.maxDecibels) flightData.maxDecibels = currentDecibels;

  // Update Max Velocity (using magnitude of IMU velocity reading)
  if (imuVelocityMag > flightData.maxVelocity) flightData.maxVelocity = imuVelocityMag;
}
...
void handleLandingSequence(float finalGravityReading[3]) {
  ...
  broadcast_string = "C KQ4FYU Temp = " + String(((currentTemp-32) * 5/9), 2) + " C, " + "Apogee = " + String((flightData.apogee * 3.28084), 2) + " ft, " +
                     /*"Battery power = " + String(battery_power) + " V, " +*/                                                                          //No battery check
                     "Max velocity = " + String((flightData.maxVelocity * 3.28084), 2) + " ft/s, " + "Flight Length = " + String((flightData.flightTime)/1000) + " s, " +  //No EST time of landing
                     "Landing Velocity = " + String((landingVelocity* 3.28084), 2) + " m/s, " + "Max G-force = " + String(flightData.maxGForce, 2) + " g, " + "Survivable = " + (calculateSurvivability() ? "True" : "False");
...
void displayFlightData() {
  ...
  if (!soar_bme.isReady() || isnan(currentTemp)) { currentTemp = soar_barometer.get_temperature(); }
...
bool calculateSurvivability() {
  ...
  return (flightData.minTemperature >= 10.0 && flightData.maxTemperature <= 35.0 && flightData.maxSusRPM <= 60.0 &&  // Check sustained RPM
