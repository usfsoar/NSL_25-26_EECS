...
bool SOAR_Lora::read(int *address, int *length, byte **data, int *rssi, int *snr) {
    *length = 0; // Default length to 0
    if (!loraSerial->available()) {
        // Serial.println("No data available on LoRa serial.");
        return false;
    }
            ...
            if (loraSerial->peek() == '+') {
                ...
                if (response.startsWith("+RCV=")) {
                    ...
                    *address = response.toInt(); // Read the address
                    ...
                    String lengthStr = loraSerial->readStringUntil(','); // Read up to the next comma
                    *length = lengthStr.toInt(); // Read the length
                    ...
                    if (*length > 0) {
                        *data = new byte[*length];
                        ...
                        while ((xTaskGetTickCount() / portTICK_PERIOD_MS) - dataStartTime < DATA_TIMEOUT && bytesRead < *length) {
                            if (loraSerial->available()) {
                        ...
                        *snr = snrStr.toInt(); // Read the SNR
                        ...
                        // Serial.println("Read message from address " + String(*address) + " with length " + String(*length) + " RSSI " + String(*rssi) + " SNR " + String(*snr));
